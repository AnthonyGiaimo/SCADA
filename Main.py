...
def getTime(session, year, month, day):
    # api-endpoint
...
def main():
    ...
    day, year, month, hour, minute, sec = currentTime.tm_mday, currentTime.tm_year, currentTime.tm_mon, \
    ...
    while not end:
        currentTime = time.localtime()
        day, year, month, hour, minute, sec = str(currentTime.tm_mday), str(currentTime.tm_year), \
                                              str(currentTime.tm_mon), str(currentTime.tm_hour), \
        ...
        if msg == "alarmSet":
            arduino_data.write(bytes(getTime(session, year, month, day), 'utf-8'))
            print("wrote to arduino")
        if msg == "exit":
            end = True;
        msg = ""
